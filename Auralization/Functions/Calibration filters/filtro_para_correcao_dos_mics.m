...
% ajustar para elas
mic11 = readmatrix('mic11_frequency_correction_final.txt');
mic10 = readmatrix('mic10_frequency_correction_final.txt');
...
% Save calibration 
[calib_filter(:,1), fs, sosMatrixL, scaleValuesL] = generate_filters(correctionL, freqVecL);
[calib_filter(:,2), ~, sosMatrixR, scaleValuesR] = generate_filters(correctionR, freqVecR);
audiowrite('Mic_calibration.wav', calib_filter, fs)

% save('calib_filter', 'sosMatrixL', 'sosMatrixR', 'scaleValuesL', 'scaleValuesR')

function [out, fs, sosMatrix, scaleValues] = generate_filters(correction, freqVec)
...
N = 24; % Ordem do filtro (vale a pena testar outros valores)
...
N_max = frfITA.samplingRate/10;
frfITA.time = frfITA.time(1:N_max);

out = frfITA.time;
fs = frfITA.samplingRate;

sosMatrix = hd1.sos; % Coeficientes [b0, b1, b2, a0, a1, a2]
scaleValues = hd1.ScaleValues; % Ganho acumulado de todos os estágios
...
% semilogx(mic10(:, 1), mic10(:, 2), '--', 'linewidth', 2);
